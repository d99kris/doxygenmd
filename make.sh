...
if [[ "${DEPS}" == "1" ]]; then
  ...
  if [ "${OS}" == "Linux" ]; then
    ...
    if [[ "${DISTRO}" == "Ubuntu" ]]; then
      sudo apt -y install bison flex || exiterr "deps failed (linux), exiting."
    else
      exiterr "deps failed (unsupported linux distro ${DISTRO}), exiting."
    fi
  elif [ "${OS}" == "Darwin" ]; then
    brew install bison flex || exiterr "deps failed (mac), exiting."
  else
...
if [[ "${DOC}" == "1" ]]; then
  if [[ -x "$(command -v help2man)" ]]; then
    ...
    help2man -n "doxygen markdown generator" -N -o ./src/doxygenmd.1 ./src/doxygenmd && ${SED} "s/\.\\\\\" DO NOT MODIFY THIS FILE\!  It was generated by help2man.*/\.\\\\\" DO NOT MODIFY THIS FILE\!  It was generated by help2man./g" ./src/doxygenmd.1 || exiterr "doc failed, exiting."
  fi
fi
